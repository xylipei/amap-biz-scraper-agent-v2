...
- **范围搜索（唯一模式）** — 所有搜索均为**严格半径圈内抓取**（不再有行政区全域 text 搜索）：中心点 = 具体地点/区县（单锚点）/城市（自动网格铺点覆盖，突破单请求 200 条上限）；半径可动态指定（如"5公里内"），**默认 5km**，高德上限 50km
...
| 输入 | Agent 行为 |
|:---|:---|
| `北京海淀区 星巴克` | 以海淀区为中心，半径 5km（默认）搜索星巴克 |
| `深圳 咖啡厅` | 自动网格铺点覆盖深圳（每圆心 5km 圈内搜索） |
| `杭州西湖区 火锅` | 以西湖区为中心，半径 5km（默认）搜索火锅店 |
| `南京鼓楼区政府大楼 周边 水果商超` | 以该地点为圆心 5km 搜索水果商超 |
| `南京站 周边 3公里 咖啡` | 半径可动态指定（3公里→3000 米） |
...
│   ├── districts.py      # 城市→区县内置表（城市名识别/推断）
│   └── config.py         # 配置读取 + 常量
...
A: 可能是搜索中心点格式问题（Agent 以该点为中心做半径搜索）。高德地理编码要求具体地名（如"南京市鼓楼区政府"），Agent 会自动规范化。如果仍失败，尝试简化中心点（如"鼓楼区"而非"浙江省杭州市西湖区"）。
