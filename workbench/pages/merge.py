...
if source_mode == "结果库（任务结果）":
    ...
    input_dir = st.text_input("CSV 目录", value=default_dir)
    if os.path.isdir(input_dir):
        ...
        source_desc = f"目录「{input_dir}」下的 {len(input_paths)} 个 CSV"

if not input_paths:
    st.warning("请至少勾选一个结果文件（或指定包含 CSV 的目录）。")
    st.stop()

st.info(f"**待合并文件**: {len(input_paths)} 个 — {source_desc}")
